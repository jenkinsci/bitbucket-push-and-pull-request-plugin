...
### Breaking Changes
* **Bitbucket Server notifications take their target and credential from the Jenkins configuration** - Build status notifications and pull request approve/decline for Bitbucket Server no longer derive the server URL from the repository links carried by the webhook payload, and the credential no longer falls back to the clone credential of the job's first Git remote. The target (*Propagation URL*) and the credential are resolved together, as a pair, from a single configuration level: the job trigger when it sets both, otherwise the global configuration when it sets both. A job for which neither level sets both does not send notifications, and the reason is reported in the controller log. Upgrade steps for Bitbucket Server setups: set the propagation URL and the credential on the same level, on the job trigger or in the global configuration. A trigger that sets only one of the two falls back to the global pair, and a setup that relied on the credential of the Git remote now needs that credential configured explicitly. Bitbucket Cloud is unchanged: the API endpoint is fixed and the credential is still resolved against the repository host.
* **The propagation URL is used as a base URL, with its context path preserved** - The pull request Server action previously reduced the configured propagation URL to `scheme://host[:port]` and dropped its path, which made it unusable for an instance served under a context path, for example behind a reverse proxy at `https://example.com/bitbucket`. The configured value is now used as the base of the REST API URLs with its path preserved, with only a trailing slash normalised, consistently with the push notification path. Upgrade steps: configure the propagation URL as the base URL of the Bitbucket instance (`https://bitbucket.example.com`, or `https://example.com/bitbucket` behind a proxy), not as a repository clone URL (`https://bitbucket.example.com/scm/PROJ/repo.git`). Setups that configured a clone URL and relied on the old stripping have to update the value.
* **API: `setBaseUrl(String)` removed, credential helpers deprecated** - `BitBucketPPRPullRequestServerAction.setBaseUrl(String)` is removed together with the `baseUrl` field it wrote: the notification base now comes from the configuration, so there is nothing left for the setter to set. `getPullRequestApiUrl()` on `BitBucketPPRAction` and its Server implementation now declare `throws MalformedURLException`. On `BitBucketPPREventContext`, `getStandardCredentials()` (no-arg), `getSecretTextCredentials()` and `getUrl()` are deprecated and kept for binary compatibility, scheduled for removal in the next major release; the Server notification path uses `getStandardCredentials(credentialsId, url)`.

**Full Changelog**: https://github.com/jenkinsci/bitbucket-push-and-pull-request-plugin/compare/bitbucket-push-and-pull-request-4.0.1...bitbucket-push-and-pull-request-4.1.0
...
**Full Changelog**: https://github.com/jenkinsci/bitbucket-push-and-pull-request-plugin/compare/bitbucket-push-and-pull-request-3.4.0...bitbucket-push-and-pull-request-4.0.0
...
**Full Changelog**: https://github.com/jenkinsci/bitbucket-push-and-pull-request-plugin/compare/bitbucket-push-and-pull-request-3.3.7...bitbucket-push-and-pull-request-3.3.8
...
**Full Changelog**: https://github.com/jenkinsci/bitbucket-push-and-pull-request-plugin/compare/bitbucket-push-and-pull-request-3.3.5...bitbucket-push-and-pull-request-3.3.6
