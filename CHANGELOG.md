...
### Bug Fixes
* **Reject malformed webhook payloads with a 4xx instead of a silent HTTP 200** - When a webhook body could not be deserialized into a usable payload (malformed, truncated, or missing a property the event needs), the receiver used to dereference it downstream — e.g. `payload.getPullRequest().getId()` — throwing a `NullPointerException` *after* the success response had already been written, so Jenkins answered HTTP 200, the job was never triggered, and the failure was silent on both sides. The receiver now (1) rejects a `null` deserialized payload at the parsing boundary and (2) builds (and thereby validates) the action *before* acknowledging the webhook, so any payload missing a required property is rejected with **HTTP 400** and a clear log instead of a dropped job. Validation is unified across all event types via the action constructors (pull request and push, cloud and server), which throw a descriptive `BitBucketPPRPayloadPropertyNotFoundException` naming the missing property. Events that have no processor (e.g. a diagnostics ping) keep their previous HTTP 200 acknowledgement, and successful payloads are still acknowledged before job matching runs. Fixes #384.
