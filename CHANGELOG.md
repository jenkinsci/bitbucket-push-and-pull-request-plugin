...
* **Warn when Bitbucket rejects a build-status notification** - A notification answered with HTTP 400 or higher (rotated token, revoked credential, wrong repository) previously left no trace at default log level, since the HTTP call itself succeeded. The plugin now logs a warning with the status code and response body, once per origin and status code, following the same anti-flooding rule as the plain-HTTP warning. Transport failures are also logged with the full exception instead of only its message.
* **Release HTTP resources after every build-status notification** - The API consumers now fully own the lifecycle of their transport: the Apache HTTP clients are closed after each call, responses are consumed through a `ResponseHandler` (which releases the connection), and the OAuth2 service and its response are closed as well. Previously every notification leaked an unclosed client with its connection manager and sockets. Callers receive a plain value (status code and body) instead of a live response object.
